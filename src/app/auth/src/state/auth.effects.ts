...
import {isBoolean, isUndefined} from '@pentegra/shared';
...
import {checkWhiteList, getUserPermissions, getUserRole} from '../helpers';
import {
  Credentials,
...
export class AuthEffects implements OnInitEffects {
  constructor(
    ...
    @Inject(WHITE_ROUTES_LIST) private whiteList: string[],
  ...
  loginSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(([action, emailAction]) => {
          let redirect = action.result.getRedirect();
          // check for super admin
          if (this.isDfe) {
            const userPermissions = getUserPermissions(
              action.result.getResponse().body
            );
            if (
              userPermissions.includes(DfePrivilege.USER_ACCOUNT_ADD) &&
              emailAction &&
              emailAction.token
            ) {
              redirect = `/auth/approve-request?token=${emailAction.token}`;
            }
          }
